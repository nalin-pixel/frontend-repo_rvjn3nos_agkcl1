...
export default function Sidebar({ active, onChange }) {
  return (
    <aside className="fixed left-0 top-0 h-screen w-16 bg-white/80 dark:bg-slate-900/80 backdrop-blur border-r border-gray-200 dark:border-slate-800 flex flex-col items-center py-4 gap-3">
      {items.map(({ key, icon: Icon, label }) => (
        <button
          ...
          className={`flex items-center justify-center w-10 h-10 rounded-xl transition-colors ${
            active === key ? 'bg-gray-900 text-white dark:bg-slate-100 dark:text-slate-900' : 'text-gray-600 dark:text-slate-300 hover:bg-gray-100 dark:hover:bg-slate-800'
          }`}
          title={label}
        >
          <Icon size={20} />
