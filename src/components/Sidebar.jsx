...
export default function Sidebar({ active, onChange }) {
  return (
    <aside className="fixed left-0 top-0 h-screen w-16 bg-white/80 backdrop-blur border-r border-slate-200 flex flex-col items-center py-4 gap-3">
      {items.map(({ key, icon: Icon, label }) => (
        <button
          ...
          className={`flex items-center justify-center w-10 h-10 rounded-xl transition-colors ${
            active === key ? 'bg-slate-900 text-white' : 'text-slate-600 hover:bg-slate-100'
          }`}
          title={label}
        >
          <Icon size={20} />
